...
def test_requirements():
    ...
    if packages_dist or packages_version:
        ...
        print('\n\nRUN THIS:\n\n', flush=True)
        print(
            f"pip uninstall peft transformers accelerate -y ; CUDA_HOME=/usr/local/cuda-11.8 pip install {' '.join(packages_all)} --upgrade",
            flush=True,
        )
        print('\n\n', flush=True)
            ...
            for req in requirements.parse(fd):
                ...
                try:
                    ...
                    if str(current_version) != str(latest_version):
                        print(f"{req.name}: {current_version} -> {latest_version}", flush=True)
