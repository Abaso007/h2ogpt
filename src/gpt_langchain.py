...
def get_llm(use_openai_model=False,
    ...
    if use_openai_model or inference_server.startswith('openai') or inference_server.startswith('vllm'):
        ...
        assert inference_server.startswith(
            'http'
        ), f"Malformed inference_server={inference_server}.  Did you add http:// in front?"
        ...
        assert not set(gen_hyper).difference(gen_kwargs.keys())
        ...
        if stream_output:
            ...
            gen_kwargs |= dict(streamer=streamer)
        else:
            streamer = None
...
def get_wiki_data(title, first_paragraph_only, text_limit=None, take_head=True):
    ...
    :return:
    """
    filename = f'wiki_{first_paragraph_only}_{title}_{text_limit}_{take_head}.data'
