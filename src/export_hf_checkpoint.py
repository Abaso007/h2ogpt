...
def do_copy(OUTPUT_NAME):
    ...
    os.system(f"""sed -i 's/from enums.*//g' {dest_file}""")
    os.system(f"""sed -i 's/from stopping.*//g' {dest_file}""")
    os.system(f"""sed -i 's/from prompter.*//g' {dest_file}""")
    os.system(
        f"""cat src/enums.py|grep -v "from enums import PromptType" >> {dest_file}"""
    )
    os.system(
        f"""cat src/prompter.py|grep -v "from enums import PromptType" >> {dest_file}"""
    )
    os.system(
        f"""cat src/stopping.py|grep -v "from enums import PromptType" >> {dest_file}"""
    )
